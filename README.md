...
```mermaid
graph LR
    A[The Challenge] --> B{Math Problem}
    A --> C{Timeline Analysis}
    A --> D{Find Liars}
    A --> E{Critical Thinking}
    
    B -->|Hallucinated| F[❌ ChatGPT: 0/25]
    B -->|Proved Impossible| G[✅ Gemini: 25/25]
    
    C -->|Missed Detail| H[⚠️ ChatGPT: 22/25]
    C -->|Perfect| I[✅ Gemini: 25/25]
    
    D -->|Found Carol| J[✅ ChatGPT: 25/25]
    D -->|Found Dan| K[✅ Gemini: 25/25]
    
    E -->|Fell Into Trap| L[⚠️ ChatGPT: 15/25]
    E -->|Identified Trick| M[✅ Gemini: 25/25]
...
```mermaid
graph TD
    Start[Which AI to Use?] --> Q1{Need Math Verification?}
    
    Q1 -->|Yes| Q2{Accuracy Critical?}
    Q1 -->|No| Q3{Need Speed?}
    
    Q2 -->|Life/Death/Money| Gemini1[💎 Use Gemini]
    Q2 -->|General Use| Q4{Complex Logic?}
    
    Q4 -->|Yes| Gemini2[💎 Use Gemini]
    Q4 -->|No| Either1[Either works]
    
    Q3 -->|Yes| Q5{Creative Task?}
    Q3 -->|No| Q6{Evidence Analysis?}
    
    Q5 -->|Yes| ChatGPT1[🤖 Use ChatGPT]
    Q5 -->|No| Q7{Conversational?}
    
    Q7 -->|Yes| ChatGPT2[🤖 Use ChatGPT]
    Q7 -->|No| Either2[Either works]
    
    Q6 -->|Hard Data| Gemini3[💎 Use Gemini]
    Q6 -->|Soft Data| ChatGPT3[🤖 Use ChatGPT]
    
    style Gemini1 fill:#a8e6cf
    style Gemini2 fill:#a8e6cf
    style Gemini3 fill:#a8e6cf
    style ChatGPT1 fill:#ffd3b6
    style ChatGPT2 fill:#ffd3b6
    style ChatGPT3 fill:#ffd3b6
    style Either1 fill:#dcedc1
    style Either2 fill:#dcedc1
```
